...
program
  ...
  .option('-P <port>', 'port of adb server', (value) => Number.parseInt(value, 10), 5037)
  .configureHelp({
    subcommandTerm(cmd) {
      let usage = cmd.usage();
...
function createDeviceCommand(nameAndArgs: string) {
  return program
    ...
    .option('-e', 'use TCP/IP device (error if multiple TCP/IP devices available)')
    .option('-s <serial>', 'use device with given serial (overrides $ANDROID_SERIAL)', process.env.ANDROID_SERIAL)
    .option('-t <id>', 'use device with given transport id', (value) => BigInt(value));
}
