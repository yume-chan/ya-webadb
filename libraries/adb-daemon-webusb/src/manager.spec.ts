            ...
            interfaces: [
              {
                ...
                alternate: {
                  alternateSetting: 0,
                  interfaceClass: AdbDefaultInterfaceFilter.classCode,
                  interfaceSubclass: AdbDefaultInterfaceFilter.subclassCode,
                  interfaceProtocol: AdbDefaultInterfaceFilter.protocolCode,
                  endpoints: []
                },
                alternates: [
                  {
...
describe('AdbDaemonWebUsbDeviceManager', () => {
  describe('requestDevice', () => {
    it('should accept 0 args', async () => {
      ...
      assert.ok((await manager.requestDevice()) instanceof AdbDaemonWebUsbDevice);
    ...
    it('should accept undefined filters', async () => {
      const usb = new MockUsb();
      const manager = new AdbDaemonWebUsbDeviceManager(usb);
      assert.ok((await manager.requestDevice({ filters: undefined })) instanceof AdbDaemonWebUsbDevice);
      assert.strictEqual(usb.requestDevice.mock.callCount(), 1);
      assert.deepEqual(usb.requestDevice.mock.calls[0]?.arguments, [
        {
          filters: [AdbDefaultInterfaceFilter],
    ...
    it('should accept empty filters array', async () => {
      const usb = new MockUsb();
      const manager = new AdbDaemonWebUsbDeviceManager(usb);
      assert.ok((await manager.requestDevice({ filters: [] })) instanceof AdbDaemonWebUsbDevice);
      assert.strictEqual(usb.requestDevice.mock.callCount(), 1);
      assert.deepEqual(usb.requestDevice.mock.calls[0]?.arguments, [
        {
          filters: [AdbDefaultInterfaceFilter],
    ...
    it('should merge missing fields with default values', async () => {
      ...
      assert.ok((await manager.requestDevice({ filters: [filter] })) instanceof AdbDaemonWebUsbDevice);
      assert.strictEqual(usb.requestDevice.mock.callCount(), 1);
      assert.deepEqual(usb.requestDevice.mock.calls[0]?.arguments, [
        {
          filters: [
