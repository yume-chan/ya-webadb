...
export function isUsbInterfaceFilter(filter: USBDeviceFilter): filter is UsbInterfaceFilter {
  return filter.classCode !== undefined && filter.subclassCode !== undefined && filter.protocolCode !== undefined;
}

function matchUsbInterfaceFilter(alternate: USBAlternateInterface, filter: UsbInterfaceFilter) {
  return (
...
export function findUsbInterface(device: USBDevice, filter: UsbInterfaceFilter): UsbInterfaceIdentifier | undefined {
...
export function matchFilter(device: USBDevice, filter: USBDeviceFilter): boolean;
export function matchFilter(device: USBDevice, filter: USBDeviceFilter): UsbInterfaceIdentifier | boolean {
  ...
  if (filter.serialNumber !== undefined && getSerialNumber(device) !== filter.serialNumber) {
    return false;
  }
