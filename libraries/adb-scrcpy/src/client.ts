...
import type { Consumable, MaybeConsumable, ReadableStream, ReadableWritablePair } from '@yume-chan/stream-extra';
import {
  AbortController,
...
export class AdbScrcpyClient {
  static async pushServer(adb: Adb, file: ReadableStream<MaybeConsumable<Uint8Array>>, filename = DefaultServerPath) {
  ...
  static async start(adb: Adb, path: string, options: AdbScrcpyOptions<Pick<ScrcpyOptions1_15.Init, 'tunnelForward'>>) {
    ...
    try {
      ...
      const stdout = process.stdout.pipeThrough(new TextDecoderStream()).pipeThrough(new SplitStringStream('\n'));
   ...
   */
  static getEncoders(adb: Adb, path: string, options: AdbScrcpyOptions<object>): Promise<ScrcpyEncoder[]> {
   ...
   * so don't reuse it elsewhere.
   */
  static getDisplays(adb: Adb, path: string, options: AdbScrcpyOptions<object>): Promise<ScrcpyDisplay[]> {
  ...
  constructor({ options, process, stdout, videoStream, audioStream, controlStream }: AdbScrcpyClientInit) {
    ...
    this.#videoStream = videoStream ? this.#createVideoStream(videoStream) : undefined;

    this.#audioStream = audioStream ? this.#createAudioStream(audioStream) : undefined;
  ...
  async #createVideoStream(initialStream: ReadableStream<Uint8Array>) {
    ...
    return {
      stream: stream.pipeThrough(this.#options.createMediaStreamTransformer()).pipeThrough(
  ...
  async #createAudioStream(initialStream: ReadableStream<Uint8Array>): Promise<AdbScrcpyAudioStreamMetadata> {
    ...
    switch (metadata.type) {
      ...
      default:
        throw new Error(`Unexpected audio metadata type ${metadata['type'] as unknown as string}`);
    }
  }
