...
import type { ScrcpyDisplay, ScrcpyEncoder, ScrcpyOptions1_16Impl, ScrcpyOptions2_0Impl } from '@yume-chan/scrcpy';
...
export class AdbScrcpyOptions2_0 extends AdbScrcpyOptions<
  ...
  Pick<ScrcpyOptions2_0Impl.Init, 'tunnelForward' | 'control' | 'sendDummyByte' | 'scid' | 'audio'>
> {
  static async getEncoders(
    adb: Adb,
    path: string,
    options: AdbScrcpyOptions<Pick<ScrcpyOptions1_16Impl.Init, 'tunnelForward'>>
  ...
  override async getEncoders(adb: Adb, path: string): Promise<ScrcpyEncoder[]> {
