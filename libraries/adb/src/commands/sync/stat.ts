...
const AdbSyncStatErrorName = /* #__PURE__ */ (() =>
  Object.fromEntries(Object.entries(AdbSyncStatErrorCode).map(([key, value]) => [value, key])))();
...
export async function adbSyncLstat(socket: AdbSyncSocket, path: string, v2: boolean): Promise<AdbSyncStat> {
...
export async function adbSyncStat(socket: AdbSyncSocket, path: string): Promise<AdbSyncStatResponse> {
  const locked = await socket.lock();
  try {
