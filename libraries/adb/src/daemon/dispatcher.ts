...
import type { ReadableWritablePair, WritableStreamDefaultWriter } from '@yume-chan/stream-extra';
...
export class AdbPacketDispatcher implements Closeable {
  ...
  async createSocket(service: string): Promise<AdbSocket> {
    ...
    const [localId, initializer] = this.#initializers.add<SocketOpenResult>();
