...
export class AdbServerClient {
  ...
  static formatDeviceService(device: AdbServerClient.DeviceSelector, command: string) {
    if (!device) {
      return `host:${command}`;
    }
  ...
  async createConnection(request: string, options?: AdbServerClient.ServerConnectionOptions): Promise<AdbServerStream> {
   ...
   */
  async trackDevices(options?: AdbServerClient.ServerConnectionOptions): Promise<AdbServerClient.DeviceObserver> {
  ...
  async reconnectDevice(device: AdbServerClient.DeviceSelector | 'offline') {
    const connection = await this.createConnection(
      device === 'offline' ? 'host:reconnect-offline' : AdbServerClient.formatDeviceService(device, 'reconnect')
    );
    try {
  ...
  async waitForDisconnect(transportId: bigint, options?: AdbServerClient.ServerConnectionOptions): Promise<void> {
   ...
   */
  async createTransport(device: AdbServerClient.DeviceSelector): Promise<AdbServerTransport> {
...
export namespace AdbServerClient {
  ...
  export interface ServerConnection extends ReadableWritablePair<Uint8Array, MaybeConsumable<Uint8Array>>, Closeable {
   ...
   */
  export type DeviceSelector = { transportId: bigint } | { serial: string } | { usb: true } | { tcp: true } | undefined;
