...
export class BugReport extends AdbCommandBase {
  ...
  static async queryCapabilities(adb: Adb): Promise<BugReport> {
    ...
    const { stderr, exitCode } = await adb.subprocess.spawnAndWait(['bugreportz', '-v']);
    ...
    const [major, minor] = match[0].split('.').map((x) => parseInt(x, 10)) as [number, number];
    return new BugReport(adb, {
  ...
  constructor(adb: Adb, capabilities: BugReportCapabilities) {
    ...
    this.#supportsBugReportZProgress = capabilities.supportsBugReportZProgress;
        ...
        stream: new WrapReadableStream({
          start: async () => {
            path = await this.bugReportZ({
              ...
              onProgress: this.#supportsBugReportZProgress ? onProgress : undefined
            });
