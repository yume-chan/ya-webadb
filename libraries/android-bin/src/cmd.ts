...
export class Cmd extends AdbCommandBase {
   ...
   */
  async spawn(shellProtocol: boolean, command: string, ...args: string[]): Promise<AdbSubprocessProtocol> {
  ...
  async spawnAndWait(command: string, ...args: string[]): Promise<AdbSubprocessWaitResult> {
