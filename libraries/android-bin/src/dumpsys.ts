...
export class DumpSys extends AdbCommandBase {
  ...
  async diskStats() {
    const output = await this.adb.subprocess.spawnAndWaitLegacy(['dumpsys', 'diskstats']);

    function getSize(name: string) {
      const match = output.match(new RegExp(`${name}-Free: (\\d+)K / (\\d+)K`));
      if (!match) {
        return [0, 0];
      }
      return [Number.parseInt(match[1]!, 10) * 1024, Number.parseInt(match[2]!, 10) * 1024];
    }
  ...
  async battery(): Promise<DumpSys.Battery.Info> {
    const output = await this.adb.subprocess.spawnAndWaitLegacy(['dumpsys', 'battery']);
    ...
    for (const line of output.split('\n')) {
      ...
      switch (field.type) {
        case 'boolean':
          ...
          break;
        case 'number':
          info[field.field] = Number.parseInt(parts[1]!.trim(), 10) as never;
          break;
        case 'string':
