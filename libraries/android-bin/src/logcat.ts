...
export type AndroidLogPriority = (typeof AndroidLogPriority)[keyof typeof AndroidLogPriority];
...
function formatNanoseconds(nanoseconds: number, modifiers: LogcatFormatModifiers) {
...
function formatTime(seconds: number, nanoseconds: number, modifiers: LogcatFormatModifiers) {
...
function formatUid(uid: number, modifiers: LogcatFormatModifiers, suffix: string) {
  return modifiers.uid ? `${uid.toString().padStart(5)}${suffix}` : '';
}

function getFormatPrefix(entry: AndroidLogEntry, format: LogcatFormat, modifiers: LogcatFormatModifiers) {
...
function formatEntryWrapLine(entry: AndroidLogEntry, format: LogcatFormat, modifiers: LogcatFormatModifiers) {
...
export async function deserializeAndroidLogEntry(stream: AsyncExactReadable): Promise<AndroidLogEntry> {
...
export class Logcat extends AdbCommandBase {
  ...
  async getLogSize(ids?: LogId[]): Promise<LogSize[]> {
    const { stdout } = await this.adb.subprocess.spawn(['logcat', '-g', ...(ids ? ['-b', Logcat.joinLogId(ids)] : [])]);

    const result: LogSize[] = [];
    for await (const line of stdout.pipeThrough(new TextDecoderStream()).pipeThrough(new SplitStringStream('\n'))) {
