...
export class OverlayDisplay extends AdbCommandBase {
  ...
  async get() {
    return OverlayDisplay.SETTING_FORMAT.parse({
      value: await this.#settings.get('global', OverlayDisplay.SETTING_KEY),
      position: 0
    }).map((device) => ({
      modes: device.modes,
  ...
  async set(devices: OverlayDisplayDevice[]): Promise<void> {
    await this.#settings.put(
      ...
      OverlayDisplay.SETTING_FORMAT.stringify(
        devices.map((device) => {
          const flags: ('secure' | 'own_content_only' | 'show_system_decorations')[] = [];
          if (device.secure) {
            flags.push('secure');
          }
