...
import { ConcatStringStream, SplitStringStream, TextDecoderStream } from '@yume-chan/stream-extra';
...
export const PACKAGE_MANAGER_INSTALL_OPTIONS_MAP: Record<keyof PackageManagerInstallOptions, string> = {
  skipExisting: '-R',
...
export const PACKAGE_MANAGER_LIST_PACKAGES_OPTIONS_MAP: Record<keyof PackageManagerListPackagesOptions, string> = {
  listDisabled: '-d',
  listEnabled: '-e',
  showSourceDir: '-f',
  showInstaller: '-i',
...
const PACKAGE_MANAGER_UNINSTALL_OPTIONS_MAP: Record<keyof PackageManagerUninstallOptions, string> = {
  keepData: '-k',
  user: '--user',
...
function buildInstallArguments(command: string, options: Partial<PackageManagerInstallOptions> | undefined): string[] {
  const args = buildArguments(['pm', command], options, PACKAGE_MANAGER_INSTALL_OPTIONS_MAP);
...
export class PackageManager extends AdbCommandBase {
   ...
   */
  async install(apks: string[], options?: Partial<PackageManagerInstallOptions>): Promise<string> {
  ...
  async pushAndInstallStream(
    ...
    try {
      const output = await this.adb.subprocess.spawnAndWaitLegacy(args.map(escapeArg)).then((output) => output.trim());
  ...
  async *listPackages(
    ...
    const args = buildArguments(['pm', 'list', 'packages'], options, PACKAGE_MANAGER_LIST_PACKAGES_OPTIONS_MAP);
    if (options?.filter) {
  ...
  async uninstall(packageName: string, options?: Partial<PackageManagerUninstallOptions>): Promise<void> {
    const args = buildArguments(['pm', 'uninstall'], options, PACKAGE_MANAGER_UNINSTALL_OPTIONS_MAP);
  ...
  async resolveActivity(options: PackageManagerResolveActivityOptions): Promise<string | undefined> {
   ...
   */
  async sessionCreate(options?: Partial<PackageManagerInstallOptions>): Promise<number> {
  ...
  async sessionAddSplit(sessionId: number, splitName: string, path: string): Promise<void> {
    const args: string[] = ['pm', 'install-write', sessionId.toString(), splitName, path];

    const output = await this.adb.subprocess.spawnAndWaitLegacy(args).then((output) => output.trim());
  ...
  async sessionAddSplitStream(
    ...
    const args: string[] = ['pm', 'install-write', '-S', size.toString(), sessionId.toString(), splitName, '-'];
...
export class PackageManagerInstallSession {
  ...
  addSplitStream(splitName: string, size: number, stream: ReadableStream<MaybeConsumable<Uint8Array>>): Promise<void> {
    return this.#packageManager.sessionAddSplitStream(this.#id, splitName, size, stream);
  }
