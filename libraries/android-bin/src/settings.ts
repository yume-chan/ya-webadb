...
export class Settings extends AdbCommandBase {
  ...
  async base(
    ...
    if (this.#cmd.supportsCmd) {
      output = await this.#cmd.spawnAndWait(command[0]!, ...command.slice(1));
    } else {
  ...
  async get(namespace: SettingsNamespace, key: string, options?: SettingsOptions): Promise<string> {
    const output = await this.base('get', namespace, options, key);
    // Remove last \n
    return output.substring(0, output.length - 1);
  }

  async delete(namespace: SettingsNamespace, key: string, options?: SettingsOptions): Promise<void> {
    await this.base('delete', namespace, options, key);
  }

  async put(namespace: SettingsNamespace, key: string, value: string, options?: SettingsPutOptions): Promise<void> {
  ...
  reset(namespace: SettingsNamespace, mode: SettingsResetMode, options?: SettingsOptions): Promise<void>;
  reset(namespace: SettingsNamespace, packageName: string, tag?: string, options?: SettingsOptions): Promise<void>;
  async reset(
    namespace: SettingsNamespace,
