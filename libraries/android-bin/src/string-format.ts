...
type SequenceResult<T extends readonly (Format<unknown> | { name: string; format: Format<unknown> })[]> = {
...
export const p = {
  ...
  sequence: <const T extends readonly (Format<unknown> | { name: string; format: Format<unknown> })[]>(
    ...args: T
    ...
    stringify: (value: SequenceResult<T>) => {
      ...
      for (const part of args) {
        if ('name' in part) {
          result += part.format.stringify(value[part.name as keyof typeof value]);
        }
      }
      return result;
    }
