...
export async function aoaHidSetReportDescriptor(device: USBDevice, accessoryId: number, reportDescriptor: Uint8Array) {
  await device.controlTransferOut(
    {
      recipient: 'device',
      requestType: 'vendor',
...
export async function aoaHidSendInputReport(device: USBDevice, accessoryId: number, event: Uint8Array) {
  await device.controlTransferOut(
    {
      recipient: 'device',
      requestType: 'vendor',
...
export class AoaHidDevice {
   ...
   */
  static async register(device: USBDevice, accessoryId: number, reportDescriptor: Uint8Array) {
