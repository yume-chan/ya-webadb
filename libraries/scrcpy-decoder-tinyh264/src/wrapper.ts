...
interface TinyH264PictureReadyMessage extends TinyH264MessageBase, PictureReadyEventArgs {
  type: 'pictureReady';
}

type TinyH264Message = TinyH264DecoderReadyMessage | TinyH264PictureReadyMessage;
...
export function createTinyH264Wrapper(): Promise<TinyH264Wrapper> {
  if (!worker) {
    ...
    worker.addEventListener('message', ({ data }: MessageEvent<TinyH264Message>) => {
      switch (data.type) {
        case 'decoderReady':
          workerReady = true;
