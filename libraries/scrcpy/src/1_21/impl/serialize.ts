...
export function serialize<T extends object>(options: T, defaults: Required<T>): string[] {
  const result: string[] = [];
  for (const [key, value] of Object.entries(options)) {
    ...
    const defaultValue = toScrcpyOptionValue(defaults[key as keyof T], undefined);
