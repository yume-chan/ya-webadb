...
class WritableStream<in T> extends NativeWritableStream<Consumable<T>> {
  static async write<T>(writer: WritableStreamDefaultWriter<Consumable<T>>, value: T) {
...
class ReadableStream<T> extends NativeReadableStream<Consumable<T>> {
  static async enqueue<T>(controller: { enqueue: (chunk: Consumable<T>) => void }, chunk: T) {
  ...
  constructor(source: Consumable.ReadableStreamSource<T>, strategy?: QueuingStrategy<T>) {
    let wrappedController: Consumable.ReadableStreamController<T> | undefined;
