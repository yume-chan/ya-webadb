...
async function testInputOutput(combine: boolean, inputLengths: number[], outputLengths: number[]) {
  ...
  for (let i = 0; i < outputLengths.length; i += 1) {
    ...
    assert.deepStrictEqual(write.mock.calls[i]!.arguments[0], TestData.subarray(offset, end));
    offset = end;
  }
}
