...
export function tryConsume<T, R>(value: T, callback: (value: T extends Consumable<infer U> ? U : T) => R): R {
...
export class WritableStream<in T> extends NativeWritableStream<MaybeConsumable<T>> {
    ...
    super(
      {
        ...
        async write(chunk, controller) {
          await tryConsume(chunk, (chunk) => sink.write?.(chunk as T, controller));
        },
        abort(reason) {
