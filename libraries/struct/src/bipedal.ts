...
function advance<T>(iterator: Iterator<unknown, T, unknown>, next: unknown): MaybePromiseLike<T> {
  while (true) {
...
export function bipedal<This, T, A extends unknown[]>(
  ...
  return function (this: This, ...args: A) {
    const iterator = fn.call(
      this,
      function* <U>(value: U | PromiseLike<U>): Generator<PromiseLike<U>, U, { resolved: U } | { error: unknown }> {
        if (isPromiseLike(value)) {
