...
export interface BufferLike {
  ...
  <U>(length: number, converter: Converter<Uint8Array, U>): Field<U, never, never>;

  <K extends string>(lengthField: K): Field<Uint8Array, K, Record<K, number>>;
  <K extends string, U>(lengthField: K, converter: Converter<Uint8Array, U>): Field<U, K, Record<K, number>>;

  <K extends string, KT>(length: BufferLengthConverter<K, KT>): Field<Uint8Array, K, Record<K, KT>>;
  <K extends string, KT, U>(
    length: BufferLengthConverter<K, KT>,
    converter: Converter<Uint8Array, U>
  ): Field<U, K, Record<K, KT>>;

  <KOmitInit extends string, KS>(length: Field<number, KOmitInit, KS>): Field<Uint8Array, KOmitInit, KS>;
...
export const buffer: BufferLike = function (
  lengthOrField: string | number | Field<number, never, unknown> | BufferLengthConverter<string, unknown>,
  ...
  if (typeof lengthOrField === 'number') {
    if (converter) {
      ...
      return {
        ...
        serialize: (value, { buffer, index }) => {
          buffer.set(converter.back(value).slice(0, lengthOrField), index);
        },
    ...
    return {
      size: lengthOrField,
      serialize: (value, { buffer, index }) => {
        buffer.set((value as Uint8Array).slice(0, lengthOrField), index);
      },
  ...
  if ((typeof lengthOrField === 'object' || typeof lengthOrField === 'function') && 'serialize' in lengthOrField) {
    if (converter) {
      return {
        ...
        dynamicSize(value) {
          ...
          const lengthFieldSize = lengthOrField.dynamicSize?.(array.length) ?? lengthOrField.size;
        ...
        deserialize: bipedal(function* (then, context) {
          ...
          const array = yield* then(context.reader.readExactly(length));
          return converter.convert(array);
        })
      };
    }
    ...
    return {
      size: 0,
      dynamicSize(value) {
        const lengthFieldSize = lengthOrField.dynamicSize?.((value as Uint8Array).length) ?? lengthOrField.size;
        return lengthFieldSize + (value as Uint8Array).length;
      },
      serialize(value, context) {
        const lengthFieldSize = lengthOrField.dynamicSize?.((value as Uint8Array).length) ?? lengthOrField.size;
        lengthOrField.serialize((value as Uint8Array).length, context);
        context.buffer.set(value as Uint8Array, context.index + lengthFieldSize);
      },
      deserialize: bipedal(function* (then, context) {
        const length = yield* then(lengthOrField.deserialize(context));
  ...
  return {
    size: 0,
    preSerialize: (value, runtimeStruct) => {
      runtimeStruct[lengthOrField.field] = lengthOrField.back((value as Uint8Array).length);
    },
    dynamicSize: (value) => {
      return (value as Uint8Array).length;
    },
