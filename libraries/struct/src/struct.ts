...
>(
  ...
  const extra = options.extra ? Object.getOwnPropertyDescriptors(options.extra) : undefined;

  return {
    fields,
    size,
    extra: options.extra,
    serialize(runtimeStruct: StructInit<Struct<T, Extra, PostDeserialize>>, buffer?: Uint8Array): Uint8Array | number {
      for (const [key, field] of fieldList) {
        if (key in runtimeStruct) {
          field.preSerialize?.(runtimeStruct[key as never], runtimeStruct as never);
        }
      }

      const sizes = fieldList.map(([key, field]) => field.dynamicSize?.(runtimeStruct[key as never]) ?? field.size);
    ...
    deserialize: bipedal(function* (this: Struct<T, Extra, PostDeserialize>, then, reader: AsyncExactReadable) {
      ...
      if (options.postDeserialize) {
        return options.postDeserialize.call(runtimeStruct as never, runtimeStruct as never);
      } else {
