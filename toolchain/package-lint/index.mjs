...
for (const p of packages) {
  ...
  if (packageJson.dependencies) {
    packageJson.dependencies = Object.fromEntries(
      Object.entries(packageJson.dependencies)
        .filter(([name]) => {
          if (packageJson.name === '@yume-chan/eslint-config' && name === 'eslint') {
            return true;
          }
  ...
  if (packageJson.devDependencies) {
    ...
    packageJson.devDependencies = Object.fromEntries(
      Object.entries(packageJson.devDependencies).filter(([name]) => !REMOVE_DEPS.includes(name))
    );

    if (packageJson.scripts) {
      packageJson.devDependencies['@yume-chan/eslint-config'] = 'workspace:^1.0.0';
      packageJson.scripts.lint = 'run-eslint && prettier src/**/*.ts --write --tab-width 4';

      if (packageJson.scripts.test) {
        packageJson.devDependencies['@yume-chan/test-runner'] = 'workspace:^1.0.0';
  ...
  if (fs.existsSync(tsConfigPath)) {
    ...
    if (fs.existsSync(testTsConfigPath)) {
      if (!tsConfig.references.some((ref) => ref.path === './tsconfig.test.json')) {
        tsConfig.references.push({ path: './tsconfig.test.json' });
      }
    } else if (tsConfig.references.some((ref) => ref.path === './tsconfig.test.json')) {
      tsConfig.references = tsConfig.references.filter((ref) => ref.path !== './tsconfig.test.json');
    }
    ...
    if (fs.existsSync(buildTsConfigPath)) {
      try {
        const buildTsConfig = JSON5.parse(fs.readFileSync(buildTsConfigPath, 'utf8'));
        if (buildTsConfig.references) {
          buildTsConfig.references.sort((a, b) => a.path.localeCompare(b.path));
        }
        fs.writeFileSync(buildTsConfigPath, JSON.stringify(buildTsConfig, null, 4) + '\n');
      } catch (e) {
        console.error('could not read buildTsConfig', buildTsConfigPath);
      }
    }
  }
}
