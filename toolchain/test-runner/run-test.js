...
let tsc = resolve(fileURLToPath(import.meta.url), '..', 'node_modules', '.bin', 'tsc');
  ...
  new Transform({
    ...
    transform(chunk, encoding, callback) {
      ...
      summary.files = summary.files.filter(
        (file) => file.path.startsWith(workingDirectory) && !file.path.endsWith('.spec.ts')
      );
      ...
      totals.coveredLinePercent = getPercentage(totals.coveredLineCount, totals.totalLineCount);
      totals.coveredBranchPercent = getPercentage(totals.coveredBranchCount, totals.totalBranchCount);
      totals.coveredFunctionPercent = getPercentage(totals.coveredFunctionCount, totals.totalFunctionCount);
...
filterCoverage.pipe(lcov).pipe(createWriteStream(resolve(coverageFolder, 'lcov.info')));
